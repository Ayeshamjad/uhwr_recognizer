    ...
    for batch_idx, batch in enumerate(progress_bar, 1):
        ...
        with autocast(device_type="cuda"):
            ...
            ctc_target_lengths = torch.tensor(
                ctc_target_lengths, device=labels.device)

            # ---- CORRECT encoder lengths (T) ----
            T = ctc_log_probs.size(0) if ctc_log_probs.dim(
            ) == 3 else ctc_log_probs.size(1)
    ...
    for batch in tqdm(data_loader, desc="Evaluating"):
        ...
        with torch.no_grad():
            if decode_mode == "greedy":
                ...
                outputs = beam_search_decode(model, pixel_values, tokenizer)
...
def main():
    ...
    train_df = pd.read_csv(f"{DATA_ROOT}/data_upti/train_icdar.csv")
    # eval_df = pd.read_csv(f"{DATA_ROOT}/data_uhwr/val.csv")
    eval_df = pd.read_csv(f"{DATA_ROOT}/data_upti/test_icdar.csv")
    ...
    for epoch in range(max_epochs):
        ...
        #     loss_fn.ce_weight = 0.1
        #     loss_fn.ctc_weight = 0.9
        #     # print("🔓 Unfroze decoder layers for fine-tuning.")
        #     print("⚖️ Adjusted loss weights: CTC=0.9, CE=0.1")
        ...
        val_loss, val_cer = evaluate(
            model, eval_loader, tokenizer, device, loss_fn
        )
        ...
        if improved:
            ...
            patience_ctr = 0
            torch.save(model.state_dict(), "best_model_upti_icdar.pt")
    ...
    artifacts_dir = os.path.join(ROOT, "training_artifacts_new")
    filename = f"loss_curve_icdar_{len(train_loss_history)}epochs.png"
    plot_path = save_loss_plot(
        train_loss_history, val_loss_history, artifacts_dir, filename)
    if plot_path:
